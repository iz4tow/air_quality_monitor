...
func main() {
	...
	for {
		...
		_, err = db.Exec(`INSERT INTO sensor_data (timestamp, temperature, humidity, co2, nh3, nox, co, pm25, pm10) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			time.Now(), data.Temperature, data.Humidity, data.CO2, data.NH3, data.NOx, data.CO, data.Dust25, data.Dust10)
		if err != nil {
